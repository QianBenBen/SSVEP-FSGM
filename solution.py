...
class Solution:
    ...
    def save_checkpoint(self, filename):
        ...
        filepath = self.ckpt_dir + "/" + filename + ".tar"
...
def eeg_train(learning_rate: float = 1e-3,
    ...
    # checkpoints保存一般要保存四点内容 ：epoch, model_state_dict, optimizer_state_dict, loss
    # 此处我们只保存了model_state_dict
    torch.save(eegnet, f"checkpoints/eeg_gpu.pth")
    # torch.save(light.state_dict(), f"checkpoints/light_{epoch}.pth")
    print("模型已保存")
    writer.close()