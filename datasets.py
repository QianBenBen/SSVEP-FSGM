...
class Benchmark(Dataset):
    ...
    def load_data2(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.train:
            # train data
            print("---------- 训练数据加载 ----------", flush=False)
            data = np.zeros((int(240 * self.subject_num * 0.8), len(channels), 1375))
            pre_data = np.zeros((int(240 * self.subject_num * 0.8), len(channels), 125))
            label = np.zeros(int(240 * self.subject_num * 0.8), dtype=int)
        else:
            # test data
            print("---------- 测试数据加载 ----------", flush=False)
            data = np.zeros((int(240 * self.subject_num * 0.2), len(channels), 1375))
            pre_data = np.zeros((int(240 * self.subject_num * 0.2), len(channels), 125))
        ...
        for sub_num in tqdm(range(start_subject, end_subject)):
            ...
            for block in range(6):
                for target in range(40):
                    data[(sub_num - start_subject) * 240 + block * 40 + target] = f["data"][channels, 125:, target, block]
                    pre_data[(sub_num - start_subject) * 240 + block * 40 + target] = f["data"][channels, :125, target,
                                                                          block]
